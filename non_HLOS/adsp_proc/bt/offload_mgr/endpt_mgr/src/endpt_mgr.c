 ...
 * - HLOS -> ADSP -> AWM offload path debug log 추가.
 * - UAPP_DATA_RX_RES 처리 활성화.
 */
...
{
    OFFLOAD_MGR_LOGM("add hyungchul:endpt_mgr_init");
}

void endpt_mgr_deinit(void)
{
    OFFLOAD_MGR_LOGM("add hyungchul:endpt_mgr_deinit");
}

void endpt_mgr_handle_offload_app_evt(endpt_mgr_rpc_header_t *rpc_msg)
{
    OFFLOAD_MGR_LOGM("add hyungchul:endpt_mgr_handle_offload_app_evt opcode=0x%x endpoint=0x%x len=%d",
                     rpc_msg->opcode,
                     (uint32_t)rpc_msg->endpoint_id,
                     rpc_msg->data_len);
    ...
    case UAPP_OPEN_SOCKET_RES:
    {
        ...
        OFFLOAD_MGR_LOGM("add hyungchul:UAPP_OPEN_SOCKET_RES status=%d socket_id lower32=%d",
                         rsp->status,
                         (uint32_t)rsp->socket_id);
    ...
    case UAPP_DATA_TX_REQ:
    {
        ...
        OFFLOAD_MGR_LOGM("add hyungchul:UAPP_DATA_TX_REQ socket_id lower32=%d len=%d",
    ...
    case UAPP_DATA_RX_RES:
    {
        ...
        OFFLOAD_MGR_LOGM("add hyungchul:UAPP_DATA_RX_RES socket_id lower32=%d",
                         (uint32_t)data_rx_rsp->socket_id);
...
void endpt_mgr_handle_awm_evt(uint8_t *evt, size_t len)
{
    ...
    OFFLOAD_MGR_LOGM("add hyungchul:AWM evt cmd_type=%d opcode=0x%x endpoint=0x%x",
                     rpc_msg->command_type,
                     rpc_msg->opcode,
                     (uint32_t)rpc_msg->endpoint_id);
...
{
    OFFLOAD_MGR_LOGM("add hyungchul:context hub evt opcode=0x%x len=%d", opcode, len);
    /* header */
...
{
    endpt_mgr_rpc_header_t *header = (endpt_mgr_rpc_header_t *)data;

    if (*hub_id == ENDPT_MGR_HUB_ID_AWM)
    {
        OFFLOAD_MGR_LOGL("hub id matched to awm : Sending msg ... ");
        OFFLOAD_MGR_LOGM("add hyungchul:send msg to AWM opcode=0x%x endpoint=0x%x len=%d",
                         header->opcode,
                         (uint32_t)header->endpoint_id,
                         data_len);
