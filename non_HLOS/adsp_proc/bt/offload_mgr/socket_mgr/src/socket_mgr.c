...
void socket_mgr_store_socket_data(void *d1, void *d2)
{
    ...
    OFFLOAD_MGR_LOGM("add hyungchul: socket_mgr_store_socket_data socket_id lower32=%d type=%d hub lower32=0x%x endpoint lower32=0x%x",
                     (uint32_t)socket_open->ctx.socket_id,
                     socket_open->ctx.type,
                     (uint32_t)socket_open->ctx.offloadEndpointId.hubID,
                     (uint32_t)socket_open->ctx.offloadEndpointId.endpointID);
}
...
void socket_mgr_get_socket_capabilities(void)
{
    ...
    OFFLOAD_MGR_LOGM("add hyungchul: socket_mgr_get_socket_capabilities enter");
    ...
    OFFLOAD_MGR_LOGM("add hyungchul: raw caps lecoc max=%d mtu=%d rfcomm max=%d frame=%d",
    ...
    OFFLOAD_MGR_LOGM("add hyungchul: send BT_OFFLOAD_SOCKET_CAPS to HLOS Socket HAL len=%d",
...
void socket_mgr_offload_req_to_profile(void *d1, void *d2) 
{
    ...
    OFFLOAD_MGR_LOGM("add hyungchul: socket_mgr_offload_req_to_profile type=%d socket_id lower32=%d",
                     socket_open->ctx.type,
                     (uint32_t)socket_open->ctx.socket_id);
    ...
    switch(socket_open->ctx.type)
    {
        case SOCKET_TYPE_LECOC:
            OFFLOAD_MGR_LOGM("add hyungchul: dispatch to profile_mgr_lecoc_socket_open");
            profile_mgr_lecoc_socket_open(socket_open, offload);
            break;
        
        case SOCKET_TYPE_RFCOMM:
            OFFLOAD_MGR_LOGM("add hyungchul: dispatch to profile_mgr_rfcomm_socket_open");
            profile_mgr_rfcomm_socket_open(socket_open, offload);
            break;
        
        default:
            OFFLOAD_MGR_LOGE("add hyungchul: invalid socket type=%d", socket_open->ctx.type);
...
void socket_mgr_unoffload_req_to_profile(void *d1, void *d2) 
{
    ...
    OFFLOAD_MGR_LOGM("add hyungchul: socket_mgr_unoffload_req_to_profile socket_id lower32=%d type=%d pseudo_id=%d",
                     (uint32_t)(*socket_id),
                     offload->offload_data.socket_data.socket_type,
...
void socket_mgr_offload_req_to_microapp(void *data) 
{
    ...
    OFFLOAD_MGR_LOGM("add hyungchul: socket_mgr_offload_req_to_microapp socket_id lower32=%d endpoint lower32=0x%x type=%d",
                     (uint32_t)offload->offload_data.socket_data.socket_id,
                     (uint32_t)offload->ep_id.id,
                     offload->offload_data.socket_data.socket_type);
    ...
    OFFLOAD_MGR_LOGM("add hyungchul: send UAPP_OPEN_SOCKET_REQ to AWM");
...
void socket_mgr_unoffload_req_to_microapp(void *data) 
{
    ...
    OFFLOAD_MGR_LOGM("add hyungchul: send UAPP_SOCKET_CLOSE_REQ to AWM socket_id lower32=%d",
                     (uint32_t)req->socket_id);
...
void socket_mgr_send_offload_resp_to_hal(void *data, int status) 
{
    ...
    OFFLOAD_MGR_LOGM("add hyungchul: socket_mgr_send_offload_resp_to_hal socket_id lower32=%d status=%d",
                     (uint32_t)socket_open_rsp.sock_id,
                     status);
...
void socket_mgr_send_unoffload_resp_to_hal(void *data, int status) 
{
    ...
    OFFLOAD_MGR_LOGM("add hyungchul: socket_mgr_send_unoffload_resp_to_hal socket_id lower32=%d status=%d",
                     (uint32_t)socket_close_rsp.sock_id,
                     status);
...
void socket_mgr_offload_to_microapp_success_cb(void *data, int status) 
{
    ...
    OFFLOAD_MGR_LOGM("add hyungchul: socket_mgr_offload_to_microapp_success_cb status=%d type=%d socket_id lower32=%d",
                     status,
                     offload->offload_data.socket_data.socket_type,
                     (uint32_t)offload->offload_data.socket_data.socket_id);
...
{
	/* nothing to do here */
    OFFLOAD_MGR_LOGM("change(add)-hyungchul-20260511-0001: socket_mgr_unoffload_from_microapp_success_cb status=%d", status);
}
...
{
    OFFLOAD_MGR_LOGM("add hyungchul: socket_mgr_init");
...
{
    OFFLOAD_MGR_LOGM("change(add)-hyungchul-20260511-0001: socket_mgr_deinit");
...
void socket_mgr_microstack_offload_cfm(void *message) 
{
    ...
    OFFLOAD_MGR_LOGM("add hyungchul: socket_mgr_microstack_offload_cfm socketId lower32=%d connId=%d resultCode=%d",
                     (uint32_t)cfm->socketId,
                     cfm->connId,
                     cfm->resultCode);
...
void socket_mgr_microstack_unoffload_cfm(void *message) 
{
    ...
    OFFLOAD_MGR_LOGM("add hyungchul: socket_mgr_microstack_unoffload_cfm connId=%d resultCode=%d",
                     cfm->connId,
                     cfm->resultCode);
...
void socket_mgr_send_microstack_socket_close_ind_to_hal(socket_id_t socket_id, int status) 
{
    ...
    OFFLOAD_MGR_LOGM("add hyungchul: socket_mgr_send_microstack_socket_close_ind_to_hal socket_id lower32=%d status=%d",
                     (uint32_t)socket_id,
                     status);
...
void socket_mgr_data_tx(uint64_t socket_id, void *data, int data_length)
{
    ...
    OFFLOAD_MGR_LOGM("add hyungchul: socket_mgr_data_tx socket_id lower32=%d len=%d",
                     (uint32_t)socket_id,
                     data_length);
...
void socket_mgr_data_rx_rsp(uint64_t socket_id)
{
    ...
    OFFLOAD_MGR_LOGM("add hyungchul: socket_mgr_data_rx_rsp socket_id lower32=%d",
                     (uint32_t)socket_id);
...
void socket_mgr_send_data_tx_cfm_to_uapp(BmmSocketDataCfm *cfm)
{
    ...
    OFFLOAD_MGR_LOGM("add hyungchul: socket_mgr_send_data_tx_cfm_to_uapp socket_id lower32=%d resultCode=%d",
                     (uint32_t)offload->offload_data.socket_data.socket_id,
                     cfm->resultCode);
	/* header */
    endpt_mgr_rpc_header_t *header = endpt_mgr_rpc_get_default_header();
...
void socket_mgr_send_data_rx_ind_to_uapp(BmmSocketDataInd *ind)
{
    ...
    OFFLOAD_MGR_LOGM("add hyungchul: socket_mgr_send_data_rx_ind_to_uapp connId=%d dataLen=%d",
                     ind->connId,
                     ind->dataLen);
...
void socket_mgr_handle_open_socket(uint8_t *evt, size_t len)
{
    ...
    OFFLOAD_MGR_LOGM("add hyungchul: socket_mgr_handle_open_socket enter len=%d", (int)len);
    ...
    if (!status)
    {
		...
		OFFLOAD_MGR_LOGE("add hyungchul: decode_socket_open failed");
        return;
    }

    /* TBD : check if the socket with socketid exists */
	OFFLOAD_MGR_LOGM("add hyungchul: decoded socket_open socket_id lower32=%d type=%d acl=%d endpoint lower32=0x%x",
                     (uint32_t)socket_open.ctx.socket_id,
                     socket_open.ctx.type,
                     socket_open.ctx.aclConnectionHandle,
                     (uint32_t)socket_open.ctx.offloadEndpointId.endpointID);

    if (socket_open.ctx.type == SOCKET_TYPE_LECOC)
    {
        OFFLOAD_MGR_LOGM("add hyungchul: decoded LECoC localCid=%d remoteCid=%d psm=%d localMtu=%d remoteMtu=%d localMps=%d remoteMps=%d rxCredits=%d txCredits=%d",
                         socket_open.ctx.channel_info.lecoc_channel_info.localCid,
                         socket_open.ctx.channel_info.lecoc_channel_info.remoteCid,
                         socket_open.ctx.channel_info.lecoc_channel_info.psm,
                         socket_open.ctx.channel_info.lecoc_channel_info.localMtu,
                         socket_open.ctx.channel_info.lecoc_channel_info.remoteMtu,
                         socket_open.ctx.channel_info.lecoc_channel_info.localMps,
                         socket_open.ctx.channel_info.lecoc_channel_info.remoteMps,
                         socket_open.ctx.channel_info.lecoc_channel_info.initialRxCredits,
                         socket_open.ctx.channel_info.lecoc_channel_info.initialTxCredits);
    }
    ...
    OFFLOAD_MGR_LOGM("add)-hyungchul-20260511-0001: call offload_mgr_sm_handle_offload_evt");
...
void socket_mgr_handle_close_socket(uint8_t *evt, size_t len)
{
    ...
    OFFLOAD_MGR_LOGM("add hyungchul: socket_mgr_handle_close_socket enter len=%d", (int)len);
    ...
    OFFLOAD_MGR_LOGM("add hyungchul: decoded close socket_id lower32=%d", (uint32_t)socket_id);
