...
/*
 * Raw DATA_TX_CFM log? fragment?? ???? UART? ??? ? ? ??.
 * JPEG demo ? [BLE_DEMO] log? 1/32/last ??? ?? ?????,
 * ???? 0?? ?? ??? ?? 1? ?? ?? CFM log? ??.
 */
#define LECOC_LOG_EVERY_TX_CFM              0U
...
void lecoc_socket_data_rx_ind(uint64_t socketId, uint16_t appDataLen,void *appData)
{
	...
	/* active "lpai_bt_appmgr_send_endpt_msg_adsp" function by hyungchul */
...
void lecoc_app_cb(uint64_t endPointId, uint16_t eventId , uint16_t appDataLen , void *appData , bool proto_encoded)
{
	...
	switch(eventId)
	{
		...
		case UAPP_SOCKET_CLOSE_CMD:
		{
            ...
            /* add hyungchul : JPEG demo에 socket close event 전달 */
...
/* change hyungchul					
					...
					else if((lecocAppInfo.socketDetails[idx].pendingOperations & TX_ENABLE)  == TX_ENABLE)
					{
                        ...
                        /* add hyungchul : JPEG demo? TX confirm ??.
                         * TX_ENABLE? ?? clear? ? ?? fragment work? schedule??
                         * qapi_bt_lecoc_send_data()? INVALID_OPERATION?? ??? ???. */
